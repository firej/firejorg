...
# Как ходить по https на локальный Home Assistant
## 1. mikrotik

Для начала нужно настроить в своём роутере DNS-запись для того, чтобы вполне валидное внешнее имя резолвилось внутри сети во внутренний локальный адрес. При этом для доступа из интернета — оно будет резолвиться во внешний адрес.
